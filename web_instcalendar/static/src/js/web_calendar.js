...
openerp.web_instcalendar = function(instance) {
        ...
        return {
            ...
            timeFormat : {
               ...
               agenda: shortTimeformat + '{ - ' + shortTimeformat + '}', // 5:00 - 6:30
                // for all other views
               '': shortTimeformat + '{ - ' + shortTimeformat + '}'.replace(/:mm/,'(:mm)'), // 5:00 - 6:30
//                '': shortTimeformat.replace(/:mm/,'(:mm)')  // 7pm
    ...
    instance.web.views.add('instcalendar', 'instance.web_instcalendar.CalendarView');

    instance.web_instcalendar.CalendarView = instance.web.View.extend({
        ...
        view_loading: function (fv) {
            ...
            this.time_start = attrs.time_start;
            this.time_stop = attrs.time_stop;
...
*/
        ...
        init_calendar: function() {
            ...
            if (!this.sidebar && this.options.$sidebar) {
                ...
                this.sidebar = new instance.web_instcalendar.Sidebar(this);
...
//			        clearInterval(interval);
        ...
        event_data_transform: function(evt) {
            ...
            if(this.time_start){
	            time_start=evt[this.time_start];
	        }
            if(this.time_stop){
	            time_stop=evt[this.time_stop];
			}
            if (this.info_fields) {
                var temp_ret = {};
            ...
            rs=[];
            for (var d = date_start.clone(); d <= date_stop; d.setDate(d.getDate() + 1)) {
	            var r = {
	                'start': new Date(d.toString('yyyy-MM-dd ')+time_start).toString('yyyy-MM-dd HH:mm:ss'),
	                'end': new Date(d.toString('yyyy-MM-dd ')+time_stop).toString('yyyy-MM-dd HH:mm:ss'),
	                'title': the_title,
	                'allDay': false,
	                'id': evt.id,
	                'attendees':attendees
    ...
    function widget_instcalendar_lazy_init() {
        if (instance.web.form.Many2ManyCalendarView) {
            return;
        }

        instance.web_instcalendar.FieldCalendarView = instance.web_instcalendar.CalendarView.extend({
    ...
    instance.web_instcalendar.BufferedDataSet = instance.web.BufferedDataSet.extend({
    ...
    instance.web_instcalendar.fields_dataset = new instance.web.Registry({
        'many2many': 'instance.web.DataSetStatic',
        'one2many': 'instance.web_instcalendar.BufferedDataSet',
    });


    function get_field_dataset_class(type) {
        var obj = instance.web_instcalendar.fields_dataset.get_any([type]);
        if (!obj) {
     ...
     */
    instance.web_instcalendar.FieldCalendar = instance.web.form.AbstractField.extend({
    ...
    instance.web_instcalendar.Sidebar = instance.web.Widget.extend({
        template: 'CalendarView.sidebar',
        
        start: function() {
            this._super();
            this.filter = new instance.web_instcalendar.SidebarFilter(this, this.getParent());
            this.filter.appendTo(this.$el.find('.oe_calendar_filter'));
        }
    });
    instance.web_instcalendar.SidebarFilter = instance.web.Widget.extend({
        events: {
